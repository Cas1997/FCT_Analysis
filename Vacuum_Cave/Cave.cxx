...
void Cave::ConstructGeometry()
{
  ...
  TGeoMaterial *matVacuum = new TGeoMaterial("Vacuum", 0,0,0); // Cas add-on. Remove when push
  TGeoMedium *kMedVac = new TGeoMedium("Vacuum", 1, matVacuum); // Cas add-on. REmove when push

  auto cavevol = gGeoManager->MakeBox("cave", kMedVac, dALIC[0], dALIC[1], dALIC[2]); // Cas add-on. Remove when push
  ...
  TGeoVolume* voBarrel = new TGeoVolume("barrel", shCaveTR, kMedVac); // Cas add-on. Remove when push
  ...
  TGeoVolume* caveRB24 = new TGeoVolume("caveRB24", shCaveRB24, kMedVac); // Cas add-on. Remove when push
