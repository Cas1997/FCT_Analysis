...
TCanvas summary_report_gen(TH2F **hists, int array_size, std::string CanvasName, std::string tlt = "Summary report"){
  TCanvas *c1 = new TCanvas(CanvasName.c_str(), CanvasName.c_str(), 1920, 1080);
  c1->UseCurrentStyle();
  ...
  bottomPad->SetPad(0.000, 0.0, 1, .95);
  int divider = 0;
  while(divider * divider < array_size){
    divider++;
  }
  bottomPad->Divide(divider, divider, 0.005, 0.005);

  for(int i = 0; i < array_size; i++){
    bottomPad->cd(i + 1);
    gPad->SetBottomMargin(0.15);
    hists[i]->Draw();
    hists[i]->SetMarkerColor(4);
    hists[i]->SetMarkerStyle(21);
    hists[i]->SetMarkerSize(1);
    TLatex entries = TLatex();
    entries.SetNDC();
    entries.SetTextSize(0.045);
    entries.SetTextAlign(11);
    entries.DrawLatex(.08, .08, Form("Max %.2f hits/cm^2/event", hists[i]->GetMaximum()));
    entries.Draw();
  }
